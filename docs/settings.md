...
| `autolearn.enabled`           | boolean | `true`        | Experimental: after the agent stops, nudge it to capture lessons to memory and create/enhance isolated managed skills under `~/.omp/agent/managed-skills`. Enables the `manage_skill` tool (and `learn` when a memory backend is active). |
| `autolearn.autoContinue`      | boolean | `true`        | When `autolearn.enabled`, auto-run one capture turn after a turn where at least one tool call errored (uses extra tokens, but far less than before -- see `captureContextTurns`). Off = only the standing passive guidance remains.       |
| `autolearn.captureContextTurns` | number | `2`         | User-turn-bounded window of the transcript resent to the capture turn, instead of the entire conversation -- the nudge only asks about the previous turn.                                                                                |
