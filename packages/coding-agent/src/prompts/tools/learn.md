...
`trigger: "correction"` when this lesson fixes something that actually failed this session (a tool call errored, an approach had to be retried) — it's weighted higher so it outranks routine notes the next time it's relevant. Omit, or use `"convention"`, for a durable fact/preference/convention noticed without a prior failure.
