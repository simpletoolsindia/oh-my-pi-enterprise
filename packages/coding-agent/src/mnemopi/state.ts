...
function createScopedResources(config: MnemopiBackendConfig): MnemopiScopedResources {
	// Env vars (MNEMOPI_ENHANCED_RECALL) still override these config-driven
	// defaults inside the core gates. Proactive linking is per-memory instance
	// below so concurrent sessions cannot clobber each other.
