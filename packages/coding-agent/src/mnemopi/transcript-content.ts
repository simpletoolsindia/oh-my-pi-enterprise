/**
 * Pure content utilities for the Mnemopi memory backend.
 *
...
const LEGACY_MEMORIES_REGEX = /<hindsight_memories>[\s\S]*?<\/hindsight_memories>/g;
 ...
 */
export function sliceLastTurnsByUserBoundary(messages: TranscriptMessage[], turns: number): TranscriptMessage[] {
 ...
 */
function formatRetentionMessage(msg: TranscriptMessage, includeTimestamps = false): string | null {
	const content = stripMemoryTags(msg.content).trim();
...
function formatRetentionMessages(messages: TranscriptMessage[], includeTimestamps = false): RetentionTranscript {
	const parts: string[] = [];
...
function formatEmbeddableRetentionMessages(messages: TranscriptMessage[]): RetentionTranscript {
	const parts: string[] = [];
	for (const msg of messages) {
...
export function prepareRetentionTranscript(
	...
	let targetMessages: TranscriptMessage[];
	if (retainFullWindow) {
...
export function prepareEmbeddableRetentionTranscript(messages: TranscriptMessage[]): RetentionTranscript {
	return formatEmbeddableRetentionMessages(messages);
}
/** Format only user-authored messages for memory fact/entity extraction. */
export function prepareUserRetentionTranscript(messages: TranscriptMessage[]): RetentionTranscript {
